...
static void on_ip_event(void *arg, esp_event_base_t base, int32_t id, void *data)
{
    if (base == IP_EVENT && id == IP_EVENT_STA_GOT_IP) {
        /* Bring up an IPv6 link-local address so mDNS has an AAAA to publish.
           Without one the device answers the A query for <hostname>.local and
           simply drops the AAAA, with no NSEC to say "there is no AAAA" — so
           macOS waits out its full 5 s .local resolver timeout on EVERY lookup
           before falling back to the A it already had. That is a flat 5 s added
           to every page load and every API call made by name; by IP it is 40 ms.
           Harmless if it fails (older netif, IPv6 compiled out): the address is
           advisory and nothing here depends on it. */
